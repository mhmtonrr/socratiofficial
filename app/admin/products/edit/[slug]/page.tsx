...
export default function EditProductPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const id = slug; // Keep id available for backwards compatibility in the variable logic
    ...
    useEffect(() => {
        ...
        Promise.all([fetchCategories, fetchProduct])
            .then(([categoriesData, allProducts]) => {
                ...
                const product = allProducts.find((p: any) => p.id === id || p.slug === id || p.name === decodeURIComponent(id));
                if (product) {
                    setFormData({
                        ...product,
