...
export default function NewProductPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        variants: [{ sku: '', size: '', color: '', colorHex: '#000000', stock: 10, price: '' }]
    });
    ...
    useEffect(() => {
        fetch('/api/admin/categories')
            .then(res => res.json())
            .then(data => {
                const level3Categories = data.filter((cat: any) => cat.parent && cat.parent.parent);
                const sorted = level3Categories.sort((a: any, b: any) => getCategoryName(a).localeCompare(getCategoryName(b)));
                setCategories(sorted);
                if (sorted.length > 0) setFormData(prev => ({ ...prev, categoryId: sorted[0].id }));
            });
    }, []);
    ...
    const handleAddImage = () => {
        setFormData(prev => ({ ...prev, images: [...prev.images, { url: '', isMain: false }] }));
    };
    ...
    const handleAddVariant = () => {
        setFormData(prev => ({ ...prev, variants: [...prev.variants, { sku: '', size: '', color: '', colorHex: '#000000', stock: 10, price: '' }] }));
    };
        ...
        <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-12 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 sticky top-0 bg-gray-50/80 backdrop-blur-md py-6 z-30 border-b border-gray-100 -mx-4 px-4 m-0">
                <div>
                    ...
                    <h1 className="text-3xl font-serif font-black text-text-main-light tracking-tight">Curate New Masterpiece</h1>
                </div>
                <div className="flex items-center gap-4">
                    <button type="button" className="px-6 py-3 text-gray-400 uppercase text-[10px] tracking-[0.2em] font-black border-2 border-gray-200 rounded-2xl flex items-center gap-2 hover:bg-white hover:border-primary/20 hover:text-primary transition-all">
                    ...
                    >
                        <Save className="w-4 h-4" /> {loading ? 'Curating...' : 'Release Collection'}
                    </button>
                </div>
            </div>
                    ...
                    <section className="bg-white rounded-[2.5rem] p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 relative overflow-hidden group">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-3">
                                ...
                                <input
                                    ...
                                    required
                                    type="text"
                                    placeholder="e.g. Midnight Suede Stiletto"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-black pl-1">Unique URL Handle</label>
                                <input
                                    name="slug"
                                    value={formData.slug}
                                    onChange={handleChange}
                        ...
                        <div className="mt-8 space-y-3">
                            ...
                            <textarea
                                ...
                                onChange={handleChange}
                                rows={5}
                                placeholder="Describe the silhouette, the inspiration, and the soul of this piece..."
                                        ...
                                        <div className="flex flex-col gap-4">
                                            <div className="flex gap-4">
                                                <input
                                                    ...
                                                    type="text"
                                                    placeholder="https://images.luxury.com/collection/..."
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        const update = formData.images.map((im, i) => ({ ...im, isMain: i === idx }));
                                                        setFormData(prev => ({ ...prev, images: update }));
                                                    }}
                    ...
                    <section className="bg-white rounded-[2.5rem] p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 sticky top-32">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-4 border-t border-gray-50 pt-8">
                                ...
                                <textarea
                                    ...
                                    rows={5}
                                    placeholder="How to maintain the eternal grace of this piece..."
                        ...
                        <div className="mt-12 bg-gray-50 p-6 rounded-3xl border border-gray-100">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="text-[9px] font-black uppercase tracking-widest text-text-main-light">Inventory Ready</span>
                            </div>
                            <p className="text-[10px] text-gray-400 leading-relaxed">By releasing this collection, it will become immediately visible in the public boutique gallery for your clientele.</p>
                        </div>
                    </section>
                </div>
            </div>
